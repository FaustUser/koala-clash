export default {
  ...
  pages: {
    ...
    dns: {
      ...
      placeholderCIDR: 'напр.: 1.1.1.1/32',
      placeholderCountryCode: 'напр.: RU',
      placeholderGeosite: 'напр.: gfw'
    },
    sniffer: {
  ...
  settings: {
    ...
    appearance: {
      ...
      floatingWindowSize: 'Размер плавающего окна',
      floatingWindowSizeSmall: 'Маленький',
      floatingWindowSizeDefault: 'Стандартный',
      ...
      customFloatingWindowSizeHelp:
        'Первое поле — ширина, второе — высота. Минимальные значения: ширина {{minWidth}}, высота {{minHeight}}. Если ввести меньше, значение будет автоматически сброшено к минимуму.',
      width: 'Ширина',
      height: 'Высота',
  ...
  dns: {
    ...
    fallbackResolver: 'Резервный резолвер',
    fallbackFilterGeoIP: 'Фильтр fallback по GeoIP',
    fallbackFilterGeoIPCode: 'Коды стран GeoIP для fallback',
    fallbackFilterDomain: 'Домены фильтра fallback',
    fallbackFilterIPCIDR: 'CIDR фильтра fallback',
    fallbackFilterGeoSite: 'GeoSite фильтра fallback',
    fallbackFilterExtra: 'Дополнительный fallback-filter (JSON)',
    fallbackFilterExtraPlaceholder: '{\n  \"my-key\": true\n}',
    fallbackFilterExtraFormatError: 'Дополнительный fallback-filter должен быть корректным JSON',
    fallbackFilterExtraObjectError: 'Дополнительный fallback-filter должен быть JSON-объектом',
    applyFallbackFilterTemplate: 'Шаблоны',
    fallbackFilterTemplateDocsCn: 'Docs / CN пример',
    fallbackFilterTemplateDocsCnDesc: 'Официальный пример Mihomo для фильтрации CN.',
    fallbackFilterTemplateLocalRu: 'Локальный / RU',
    fallbackFilterTemplateLocalRuDesc: 'Оставляет RU-трафик локально через GeoIP код RU.',
    fallbackFilterTemplateLocalUs: 'Локальный / US',
    fallbackFilterTemplateLocalUsDesc: 'Оставляет US-трафик локально через GeoIP код US.',
    fallbackFilterTemplateGlobalServices: 'Глобальные сервисы',
  ...
  profile: {
    ...
    editRules: {
      ...
      ruleTypeDescriptions: {
        ...
        'DOMAIN-SUFFIX': 'Совпадение по суффиксу домена, включая поддомены.',
        'DOMAIN-KEYWORD': 'Совпадение доменов по наличию ключевого слова.',
        'DOMAIN-REGEX': 'Совпадение доменов по регулярному выражению.',
        GEOSITE: 'Совпадение доменов из категории geosite.',
        GEOIP: 'Совпадение IP-адресов назначения по стране или региону.',
        'SRC-GEOIP': 'Совпадение исходных IP-адресов по стране или региону.',
        'IP-ASN': 'Совпадение IP-адресов назначения по ASN.',
        'SRC-IP-ASN': 'Совпадение исходных IP-адресов по ASN.',
        'IP-CIDR': 'Совпадение IP-адресов назначения по диапазону CIDR.',
        'IP-CIDR6': 'Совпадение IPv6-адресов назначения по диапазону CIDR.',
        'SRC-IP-CIDR': 'Совпадение исходных IP-адресов по диапазону CIDR.',
        'IP-SUFFIX': 'Совпадение IP-адресов назначения по суффиксному диапазону.',
        'SRC-IP-SUFFIX': 'Совпадение исходных IP-адресов по суффиксному диапазону.',
        'SRC-PORT': 'Совпадение по исходному порту соединения.',
        'DST-PORT': 'Совпадение по порту назначения соединения.',
        'IN-PORT': 'Совпадение по входящему listening-порту.',
        DSCP: 'Совпадение пакетов по значению DSCP.',
        'PROCESS-NAME': 'Точное совпадение имени процесса.',
        'PROCESS-PATH': 'Точное совпадение пути к исполняемому файлу процесса.',
        'PROCESS-NAME-REGEX': 'Совпадение имён процессов по регулярному выражению.',
        'PROCESS-PATH-REGEX': 'Совпадение путей процессов по регулярному выражению.',
        NETWORK: 'Совпадение по транспортному протоколу.',
        UID: 'Совпадение по идентификатору пользователя ОС.',
        'IN-TYPE': 'Совпадение по типу входящего подключения.',
        'IN-USER': 'Совпадение по имени аутентифицированного входящего пользователя.',
        'IN-NAME': 'Совпадение по имени входящего подключения.',
        'SUB-RULE': 'Применяет вложенное выражение подправила.',
        'RULE-SET': 'Совпадение записей из внешнего rule-set провайдера.',
        AND: 'Совпадение только если совпали все вложенные правила.',
        OR: 'Совпадение если сработало хотя бы одно вложенное правило.',
        NOT: 'Инвертирует результат вложенного правила.',
        MATCH: 'Резервное правило, которое совпадает со всем.'
      }
    },
    openFile: 'Открыть файл',
    delete: 'Удалить',
